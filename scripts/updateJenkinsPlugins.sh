...
case $1 in
  all)
    echo "updating all installed plugins on $JENKINS_POD"
    kubectl -n cicd exec -it $JENKINS_POD -- sh -c "java -jar /var/jenkins_home/war/WEB-INF/jenkins-cli.jar -s http://127.0.0.1:8080/ -auth $JENKINS_USER:$JENKINS_PASSWORD list-plugins | grep -e ')$' | awk '{ print $1 }'"
    ;;
  just-perfsig)
