...
# usage: must add deployent type
# deployment types include: ocp eks gcp aks
export DEPLOYMENT=$1
...
# validate that have oc configured first
if [ $DEPLOYMENT == ocp ]; then
  if [ -z oc ]; then
    echo "oc not installed, please install openshift commandline client to continue"
    exit 1
  fi
fi
